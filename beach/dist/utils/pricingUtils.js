...
exports.getPricesForSlot = getPricesForSlot;
exports.validatePricingRules = validatePricingRules;
 ...
 */
function calculatePrice(pricingRules, date, startTime, duration) {
 ...
 */
function getPricesForSlot(pricingRules, date, startTime) {
    ...
    if (pricingRules.periodOverrides?.enabled && pricingRules.periodOverrides.periods.length > 0) {
        const periodOverride = pricingRules.periodOverrides.periods.find((p) => date >= p.startDate && date <= p.endDate);
        if (periodOverride) {
 ...
 */
function validatePricingRules(pricingRules) {
    const errors = [];
        ...
        for (let i = 0; i < periods.length; i++) {
            for (let j = i + 1; j < periods.length; j++) {
                ...
                if ((p1.startDate <= p2.endDate && p1.endDate >= p2.startDate) ||
                    ...
                    errors.push(`Period overrides: sovrapposizione tra "${p1.label}" e "${p2.label}"`);
                }
            }
        }
    }
        ...
        for (let i = 0; i < genericSlots.length; i++) {
            for (let j = i + 1; j < genericSlots.length; j++) {
                ...
                if (timeSlotsOverlap(s1.start, s1.end, s2.start, s2.end)) {
                    errors.push(`Time slots generici: sovrapposizione tra "${s1.label}" e "${s2.label}"`);
                }
            }
        }
        ...
        for (let i = 0; i < specificSlots.length; i++) {
            for (let j = i + 1; j < specificSlots.length; j++) {
                ...
                if (commonDays.length > 0 && timeSlotsOverlap(s1.start, s1.end, s2.start, s2.end)) {
                    errors.push(`Time slots specifici: sovrapposizione tra "${s1.label}" e "${s2.label}" per i giorni ${commonDays.join(", ")}`);
                }
            }
        }
    }
 ...
 */
function getPricingPreview(pricingRules, date, startHour = 0, endHour = 24) {
    const preview = [];
    let currentLabel = "";
    ...
    for (let hour = startHour; hour < endHour; hour++) {
        ...
        if (pricingRules.timeSlotPricing?.enabled && label === (pricingRules.mode === "flat" ? "Prezzo fisso" : "Prezzo base")) {
            ...
            const slot = pricingRules.timeSlotPricing.slots.find((s) => s.daysOfWeek &&
                s.daysOfWeek.includes(dayOfWeek) &&
                isTimeInRange(time, s.start, s.end)) ||
                pricingRules.timeSlotPricing.slots.find((s) => (!s.daysOfWeek || s.daysOfWeek.length === 0) && isTimeInRange(time, s.start, s.end));
            if (slot) {
                label = slot.label;
            }
        }
