...
exports.updateCalendarSlot = exports.reopenCalendarDay = exports.closeCalendarDay = exports.deleteCampo = exports.updateCampo = exports.getCampoCalendarByMonth = exports.getCampoById = exports.createCampi = exports.getAllCampiByStruttura = exports.getCampiByStruttura = void 0;
...
const generateAnnualCalendarForCampo = async (campo, year = new Date().getFullYear()) => {
    const days = [];
    ...
    await campoCalendarDay_1.default.bulkWrite(operations);
...
===================================================== */
const getAllCampiByStruttura = async (req, res) => {
    try {
...
exports.getAllCampiByStruttura = getAllCampiByStruttura;
/* =====================================================
   POST /campi (OWNER)
   Crea campi + genera calendario annuale automatico
===================================================== */
const createCampi = async (req, res) => {
    try {
        ...
        const campiToCreate = campi.map((c) => ({
        ...
        const createdCampi = await Campo_1.default.insertMany(campiToCreate);
...
exports.createCampi = createCampi;
/* =====================================================
   GET /campi/:id (PUBLIC)
===================================================== */
...
exports.getCampoById = getCampoById;
/* =====================================================
...
const updateCampo = async (req, res) => {
    try {
        ...
        const campo = await Campo_1.default.findById(id).populate("struttura");
        if (!campo) {
            return res.status(404).json({ message: "Campo non trovato" });
        }
        // Verifica ownership
        ...
        if (surface !== undefined)
            campo.surface = surface;
        if (maxPlayers !== undefined)
            campo.maxPlayers = maxPlayers;
        if (indoor !== undefined)
            campo.indoor = indoor;
        if (pricePerHour !== undefined)
            campo.pricePerHour = pricePerHour;
        if (isActive !== undefined)
            campo.isActive = isActive;
...
exports.updateCampo = updateCampo;
/* =====================================================
...
const deleteCampo = async (req, res) => {
    try {
        ...
        await campoCalendarDay_1.default.deleteMany({ campo: req.params.id });
        console.log("✅ Calendario eliminato");
        await Campo_1.default.findByIdAndDelete(req.params.id);
...
const closeCalendarDay = async (req, res) => {
    try {
        ...
        if (campo.struttura.owner.toString() !== ownerId) {
            return res.status(403).json({ message: "Non autorizzato" });
        }
        // Trova il giorno nel calendario
        let calendarDay = await campoCalendarDay_1.default.findOne({
            campo: campoId,
            date,
        });
        if (!calendarDay) {
        ...
        const bookingsToCancel = await Booking_1.default.find({
            campo: campoId,
            date,
            status: "confirmed",
        });
...
const updateCalendarSlot = async (req, res) => {
    try {
        ...
        if (enabled === false) {
            const bookingToCancel = await Booking_1.default.findOne({
                campo: campoId,
                date,
                startTime: time,
                status: "confirmed",
