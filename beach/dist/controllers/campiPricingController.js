...
exports.validateCampoPricingRules = exports.getCampoPricing = exports.updateCampoPricing = void 0;
 ...
 */
const updateCampoPricing = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const validationErrors = (0, pricingUtils_1.validatePricingRules)(pricingRules);
        ...
        campo.pricingRules = pricingRules;
...
exports.updateCampoPricing = updateCampoPricing;
/**
 * 📊 GET PRICING DI UN CAMPO
 * GET /campi/:id/pricing
 */
const getCampoPricing = async (req, res) => {
    try {
        const { id } = req.params;
        const campo = await Campo_1.default.findById(id).select("pricingRules sport name pricePerHour");
        if (!campo) {
            return res.status(404).json({ message: "Campo non trovato" });
        }
        // Se il campo non ha pricingRules, crea una struttura di default
        let pricingRules = campo.pricingRules;
        if (!pricingRules || !pricingRules.mode) {
            const basePrice = campo.pricePerHour || 20;
            pricingRules = {
                mode: "flat",
                flatPrices: {
                    oneHour: basePrice,
...
exports.getCampoPricing = getCampoPricing;
/**
 * 🔍 VALIDA LE REGOLE DI PRICING (endpoint pubblico per testing)
 * POST /campi/pricing/validate
 */
const validateCampoPricingRules = async (req, res) => {
    try {
        const { pricingRules } = req.body;
        if (!pricingRules) {
            return res.status(400).json({ message: "pricingRules richiesto" });
        }
        const errors = (0, pricingUtils_1.validatePricingRules)(pricingRules);
        if (errors.length > 0) {
            return res.status(400).json({
                valid: false,
                errors,
...
exports.validateCampoPricingRules = validateCampoPricingRules;
