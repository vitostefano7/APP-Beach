...
export const createNotificationEndpoint = async (
  ...
  try {
    const userId = req.user!.id;
    const { recipientId, type, message, title, relatedMatch, relatedBooking } = req.body;

    console.log('📥 [Backend] Ricevuta richiesta creazione notifica:', {
      sender: userId,
      recipientId,
      type,
    ...
    if (!recipientId || !type || !message) {
      console.log('⚠️ [Backend] Parametri mancanti per notifica');
      return res.status(400).json({ message: "recipientId, type e message sono richiesti" });
    }

    console.log('💾 [Backend] Creazione notifica nel database');
    const notification = new Notification({
      ...
      message,
      relatedId: relatedMatch || relatedBooking,
      relatedModel: relatedMatch ? "Match" : relatedBooking ? "Booking" : undefined,
      isRead: false,
      createdAt: new Date()
    });
    ...
    // Invia push notification se disponibile
    try {
      const recipient = await User.findById(recipientId);
      if (recipient && recipient.expoPushToken) {
        console.log('📤 [Backend] Invio push notification');
      ...
      console.error('⚠️ [Backend] Errore invio push notification:', pushError);
    }
    ...
    console.error("❌ [Backend] Errore creazione notifica:", error);
    res.status(500).json({ message: "Errore server" });
  }
};
